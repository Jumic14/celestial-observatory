...
export class SolarSystemService {
  ...
  getCelestialObjectsByCategory(
    categoryId: string
  ): Observable<CelestialBody[]> {
    const params = new HttpParams()
      .set(
        'data',
        'id,bodyType,name,englishName,moons,semimajorAxis,perihelion,aphelion,eccentricity,inclination,density,gravity,escape,meanRadius,equaRadius,polarRadius,dimension,sideralOrbit,sideralRotation,aroundPlanet,discoveredBy,discoveryDate,alternativeName,avgTemp'
      )
  ...
  getMoons(id: string): Observable<CelestialBody> {
    return this.http.get<any>(this.apiUrl + id).pipe(
      map((data: any) => {
        const celestialBody: CelestialBody = {
          ...
          moons: data.moons
            ? data.moons.map((moonData: any) => moonData.moon)
            : [],
